...
/** NowCook Parser Stub — v1.5.1 (golden)
 *  - Attended min = 1m policy
 *  - Advisory planned mins + readiness defaults
 *  - Expanded synonyms (bring_to_boil, pasta “cook …”, drain)
 *  - Ingredient binding + emergent outputs for prep verbs
 *  - Auto dependencies (ingredient-driven + sequential fallback)
 *  - Collapse plating actions into single “Plate and finish” (5m)
 */
...
const CANONICAL_VERBS = [
  ...
  { name: "heat_oil",     attention: "attended" }, // used for advisory defaults
...
const VERB_SYNONYMS = [
  ...
  // bring to a boil (expanded)
  { canon: "bring_to_boil", patterns: [
    "bring to a boil",
  ...
  // drain
  { canon: "drain", patterns: ["drain", "drain well", "drain the pasta", "drain pasta"] }
];
...
/* --- Advisory/meta detection (don’t hide actionable lines) --- */
const isAdvisory = (s) => {
...
/* --- tokenizer & simple lemmatizer for ingredients --- */
...
/* --- find verbs in a line (for splitting) --- */
...
/* --- split a prose line into action chunks --- */
...
const DEFAULT_READINESS = {
  ...
  heat_oil: { type: "activity", value: "bubbling" }
};

function applyVerbPlannedDefaults(task) {
  let next = { ...task };

  if (next.planned_min == null) {
    const adv = DEFAULT_PLANNED_MIN[next.canonical_verb];
    if (adv) next.planned_min = adv;
  }

  if (!next.readiness_signal) {
    const r = DEFAULT_READINESS[next.canonical_verb];
    if (r) next.readiness_signal = r;
  }
...
const lineToTask = (text) => {
  ...
  const base = {
    ...
    planned_min: toDurationObj(dur)?.value ?? null, // may be filled by policy
...
function buildDependencies(tasks) {
  const producersByIng = {}; // ing_id -> [producer task ids]
...
function buildMealMap(raw) {
  ...
  // collapse finishing into single plate step
  const tasks = collapsePlating(withDeps);

  // ensure last step has dep if none
...
8. Plate and finish.`;
  ...
  const updateVerb = (taskId, newVerb) => {
    setState((prev) => {
      ...
      if (t) {
        t.canonical_verb = newVerb;
        const vMeta = CANONICAL_VERBS.find((v) => v.name === newVerb);
        t.requires_driver = vMeta ? vMeta.attention === "attended" : t.requires_driver;
        t.self_running_after_start = vMeta ? vMeta.attention === "unattended_after_start" : t.self_running_after_start;
        const enforced = applyAttendedDurationPolicy(t);
        Object.assign(t, enforced);
        const withDefaults = applyVerbPlannedDefaults(t);
        Object.assign(t, withDefaults);
      }
      return next;
    });
  };
  ...
  const updateDurationMinutes = (taskId, minutes) => {
    setState((prev) => {
      ...
      const t = next.meal.tasks.find((x) => x.id === taskId);
      if (t) {
        if (minutes == null) { t.duration_min = null; t.planned_min = null; }
        else { t.duration_min = { value: Math.max(1, minutes) }; t.planned_min = Math.max(1, minutes); }
        const enforced = applyAttendedDurationPolicy(t);
        Object.assign(t, enforced);
        const withDefaults = applyVerbPlannedDefaults(t);
        Object.assign(t, withDefaults);
