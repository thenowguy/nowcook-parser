...
/** NowCook Parser Stub — Golden Copy v1.5.0
 *  - Canonical verbs + synonyms (incl. add-water family)
 *  - Attended-minimum policy (>= 1 min)
 *  - Advisory planned durations for timeline when recipe omits time
 *  - Optional readiness defaults (boil → bubbling, etc.)
 *  - Multi-action splitting + ingredient binding
 *  - Ingredient-driven dependencies + sequential fallback
 *  - Plating collapse
 *  - Inline editors for verb & duration; JSON export
 *
 *  ANCHOR: TOP OF FILE
 */

// ======================================================================
// Canonical verbs (name + attention class)
// ======================================================================
const CANONICAL_VERBS = [
  ...
  { name: "heat_oil", attention: "attended" }, // new
...
// ======================================================================
// Synonyms → canonical verb mapping (plain substring match; ordered)
// ======================================================================
const VERB_SYNONYMS = [
  ...
  // blend/purée
  { canon: "blend", patterns: ["blend until smooth", "blend until very smooth", "purée until smooth", "puree until smooth", "purée", "puree", "process until smooth", "use an immersion blender"] },

  // knead
  { canon: "knead", patterns: ["knead", "knead the dough", "work the dough", "knead until smooth", "knead until elastic"] },

  // reduce / cook down
  { canon: "reduce", patterns: ["reduce", "reduce by", "cook down", "boil down", "simmer until reduced", "simmer down"] },
...
const READINESS_TABLE = [
  ...
  // extras we added
  { type: "consistency", value: "thickened", re: /until\s+thickened|thickens?\b/i },
  { type: "consistency", value: "smooth", re: /until\s+smooth|very\s+smooth/i },
  { type: "peaks", value: "soft_peaks", re: /soft\s+peaks?/i },
  { type: "peaks", value: "stiff_peaks", re: /stiff\s+peaks?/i }
];

// ======================================================================
// Advisory defaults (planned minutes & readiness) when recipe omits time
// ======================================================================
const DEFAULT_PLANNED_MIN = {
  ...
  // oven cooking (advisory when author omitted time)
  roast: 30,
  bake: 30,

  // short attended moves
  whisk: 1,
  stir: 1,
  toss: 1,
  season: 1,
...
// Optional readiness defaults if none were parsed
const DEFAULT_READINESS = {
  bring_to_boil: { type: "activity", value: "bubbling" },
  simmer: { type: "consistency", value: "smooth" },
  heat_oil: { type: "activity", value: "bubbling" } // placeholder; UI shows as text
};
...
const toDurationObj = (dur) =>
  !dur ? null : Object.prototype.hasOwnProperty.call(dur, "value") ? { value: dur.value } : { range: dur.range };
...
const tokenize = (s) =>
  s.toLowerCase().replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter((t) => t && !STOP.has(t)).map(singularize);
...
function applyVerbPlannedDefaults(task) {
  const hasExplicitDuration = !!task.duration_min;

  if (!hasExplicitDuration && task.planned_min == null) {
    const adv = DEFAULT_PLANNED_MIN[task.canonical_verb];
    if (typeof adv === "number" && adv > 0) {
      task.planned_min = task.requires_driver ? Math.max(1, adv) : adv;
    }
  }

  if (!task.readiness_signal && DEFAULT_READINESS[task.canonical_verb]) {
    task.readiness_signal = DEFAULT_READINESS[task.canonical_verb];
  }
...
// Ingredient word whitelists per verb
const VERB_WHITELIST = {
  ...
  whisk: ["egg", "eggs", "yolk", "yolks", "cream", "dressing", "sauce", "custard"],
  blend: ["soup", "sauce", "tomato", "tomatoes", "pepper", "peppers", "onion", "onions"],
...
const PREP_VERBS = new Set([
  ...
  "blend",
  "knead",
  "drain" // emergent outputs like "drained pasta"
]);
const participle = (v) =>
  ({
    chop: "chopped",
...
function bindIngredientsToTask(task, ingredients) {
  ...
  for (const ing of ingredients) {
    const headHit = lower.includes(` ${ing.head} `) || lower.endsWith(` ${ing.head}`) || lower.startsWith(`${ing.head} `);
...
const lineToTask = (text) => {
  ...
  const base = {
    ...
    planned_min: toDurationObj(dur)?.value ?? null, // may be filled by policies
  ...
  let t = applyAttendedDurationPolicy(base);
  t = applyVerbPlannedDefaults(t);
  return t;
};

// Segment raw text → title/author/ingredients/instructions
...
function collapsePlating(tasks) {
  ...
  const plateTask = applyAttendedDurationPolicy({
    ...
    duration_min: { value: DEFAULT_PLANNED_MIN.plate },
    planned_min: DEFAULT_PLANNED_MIN.plate,
...
// Dependencies (ingredient-driven + sequential fallback)
...
function buildDependencies(tasks) {
  ...
  const withDeps = tasks.map((t, idx) => {
    ...
    return deps.size ? { ...t, depends_on: Array.from(deps) } : t;
  });
...
function buildMealMap(raw) {
  ...
  // enforce policies again (harmless, ensures planned mins exist)
  tasks0 = tasks0.map(applyAttendedDurationPolicy).map(applyVerbPlannedDefaults);
  ...
  // ensure final has dependency if missing
  const finalTasks = tasks.map((t, i) => {
    if (!t.depends_on && SEQUENTIAL_FALLBACK && i > 0) {
      return { ...t, depends_on: [tasks[i - 1].id] };
    }
    return t;
  });

  // warnings for cooking verbs lacking duration/readiness
  const warnings = [];
...
4. Season the chicken and vegetables with salt, paprika, thyme and pepper.
5. Drizzle with olive oil and use your hands to coat the chicken and vegetables evenly. Stuff the lemon slices into the cavity of the chicken. You can truss the chicken at this point if you wish (optional).
...
7. Cook for a further 45 minutes. Allow the chicken and vegetables to rest with the heat off for at least 15 minutes.
8. Plate and finish.`;
  ...
  const updateVerb = (taskId, newVerb) => {
    setState((prev) => {
      ...
      if (idx >= 0) {
        ...
        updated = applyAttendedDurationPolicy(updated);
        updated = applyVerbPlannedDefaults(updated);

        next.meal.tasks = [...next.meal.tasks.slice(0, idx), updated, ...next.meal.tasks.slice(idx + 1)];
      }
      return next;
    });
  };
  ...
  const updateDurationMinutes = (taskId, minutes) => {
    setState((prev) => {
      const next = JSON.parse(JSON.stringify(prev));
      const t = next.meal.tasks.find((x) => x.id === taskId);
      if (t) {
        if (minutes == null) {
          t.duration_min = null;
          t.planned_min = null;
        } else {
          t.duration_min = { value: Math.max(1, minutes) };
          t.planned_min = Math.max(1, minutes);
        }
        let enforced = applyAttendedDurationPolicy(t);
        enforced = applyVerbPlannedDefaults(enforced);
        Object.assign(t, enforced);
      }
      return next;
    });
  };
  ...
  const exportJson = () => {
    ...
    a.href = url;
    a.download = `${state.meal.meal.title.replace(/\s+/g, "_")}.mealmap.json`;
    a.click();
    ...
    <div style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr", gap: 16, padding: 16 }}>
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 12, padding: 12 }}>
        ...
        <div style={{ border: "1px solid #eee", borderRadius: 8, padding: 8, margin: "12px 0" }}>
          <div>
            <b>Title:</b> {state.meal.meal.title}
          </div>
          <div>
            <b>Author:</b> {state.meal.meal.author?.name}
          </div>
        </div>
          ...
          <ol style={{ paddingLeft: 16 }}>
            {state.meal.tasks.map((t, idx) => (
              <li key={t.id} style={{ border: "1px solid #eee", borderRadius: 8, padding: 8, margin: "8px 0" }}>
                <div>
                  <b>Step {idx + 1}:</b> {t.name}
                </div>
                <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap" }}>
                  <Chip>
                    verb: <EditableVerb value={t.canonical_verb} onChange={(v) => updateVerb(t.id, v)} />
                  </Chip>
                  <Chip>
                    driver: {t.requires_driver ? "attended" : t.self_running_after_start ? "self-running after start" : "unattended"}
                  </Chip>
                  <Chip>
                    duration:&nbsp;
                    <DurationEditor duration={t.duration_min} onChangeMinutes={(mins) => updateDurationMinutes(t.id, mins)} />
                  </Chip>
                  <Chip>planned: {t.planned_min ?? "—"} min</Chip>
                  <Chip>
                    readiness: {t.readiness_signal ? `${t.readiness_signal.type}:${t.readiness_signal.value}` : "—"}
                  </Chip>
                  {t.inputs?.length ? <Chip>inputs: {t.inputs.join(", ")}</Chip> : null}
                  {t.outputs?.length ? <Chip>outputs: {t.outputs.join(", ")}</Chip> : null}
                  {t.depends_on?.length ? <Chip>depends_on: {t.depends_on.join(", ")}</Chip> : <Chip>depends_on: —</Chip>}
                </div>
              </li>
            ))}
          </ol>
